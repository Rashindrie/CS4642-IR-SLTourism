...
class YamuSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for href in response.xpath('//a[@class="front-group-item item"]/@href').extract():
            print href
            yield scrapy.Request(href, self.parse_restaurant)

        for i in range(1, 45):
            yield scrapy.Request('https://www.yamu.lk/place/restaurants?page=%s' % str(i))
    ...
    def parse_restaurant(self, response):
        ...
        restaurant.add_value(field_name='category', value='restaurant')
        ...
        restaurant.add_value(field_name='facilities', value=get_facilities())
