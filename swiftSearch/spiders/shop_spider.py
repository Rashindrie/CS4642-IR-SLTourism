...
class YamuSpider(scrapy.Spider):
    ...
    start_urls = [
        'https://www.yamu.lk/place/shops?page=1'
    ]
    ...
    def parse(self, response):
        ...
        for href in response.xpath('//a[@class="front-group-item item"]/@href').extract():
            print href
            yield scrapy.Request(href, self.parse_shop)

        for i in range(2, 15):
            yield scrapy.Request('https://www.yamu.lk/place/shops?page=%s' % str(i))
    ...
    def parse_shop(self, response):
        ...
        shop = ItemLoader(item=YamuItem(), response=response)
        shop.add_xpath(field_name='name', xpath='//div[@class="place-title-box"]/h2/text()')
        shop.add_xpath(field_name='description', xpath='//p[@class="excerpt"]/text()')
        shop.add_xpath(field_name='cuisine', xpath='//a[contains(@href,"/cuisine/")]/text()')
        shop.add_xpath(field_name='price_range', xpath='//a[contains(text(),"Rs.")]/text()')
        shop.add_xpath(field_name='dish_types', xpath='//a[contains(@href,"/dishtype/")]/text()')
        shop.add_value(field_name='category', value='shop')
        shop.add_value(field_name='overall_rating',
        ...
        shop.add_value(field_name='quality_rating',
        ...
        shop.add_value(field_name='service_rating',
        ...
        shop.add_value(field_name='ambiance_rating',
        ...
        shop.add_xpath(field_name='address', xpath='//p[@class="addressLine"]/text()')
        shop.add_value(field_name='contact',
        ...
        shop.add_value(field_name='facilities', value=get_facilities())

        yield shop.load_item()
